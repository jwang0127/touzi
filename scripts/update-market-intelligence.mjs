...
const TOPICS=[
  {category:"A股与政策",match:["a股","中国股市","沪深","证监会","chinese stock","china stock","shanghai stock","shenzhen stock"],topics:["A股","政策"],industries:["全市场"]},
...
const combinedQuery='("Chinese stock market" OR geopolitics OR sanctions OR oil OR OPEC OR gold OR copper OR bitcoin OR cryptocurrency OR semiconductor OR robotics)';
try{
  const articles=await fetchGdeltArticles(combinedQuery,50);
  for(const article of articles){const title=String(article.title||"").toLowerCase();const ranked=TOPICS.map(topic=>({topic,score:topic.match.filter(term=>title.includes(term)).length})).sort((a,b)=>b.score-a.score);if(ranked[0]?.score>0)fresh.push(enrichNews(article,ranked[0].topic,now));}

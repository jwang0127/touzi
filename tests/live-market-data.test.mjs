...
test("GitHub Pages build is anonymous and uses public market data directly", () => {
  ...
  for (const label of ["每日三股", "明日预测涨幅", "09:00", "科创板 688", "北交所 4/8/9", "今日决策", "买入", "持有", "卖出", "今日收盘推演", "实时五档盘口", "自动刷新", "确认时间与价格", "失效条件"]) {
    assert.ok(page.includes(label), `missing ${label}`);
  }
});

test("daily forecast is generated by the 09:00 Asia Shanghai schedule", () => {
  const workflow = read(".github", "workflows", "daily-picks.yml");
  const generator = read("scripts", "update-daily-picks.mjs");
  const snapshot = JSON.parse(read("docs", "daily-picks.json"));
  assert.match(workflow, /cron: "0 1 \* \* 1-5"/);
  assert.match(generator, /!\/\^688\//);
  assert.match(generator, /!\/\^\[489\]\//);
  assert.equal(snapshot.picks.length, 3);
  assert.ok(snapshot.picks.every(pick => !/^688|^[489]/.test(pick.code)));
});
